...
def greedy(file_root):
    ...
    ### 数据预处理 ###
    # 按流的起始时间排序,等待队列
    # flows.sort(key=(lambda flow:flow.bandwidth/flow.sendT),reverse=False) 
    # flows.sort(key=(lambda flow: (flow.arriveT * flow.bandwidth/(flow.sendT**3))),reverse=False) 
    flows.sort(key=(lambda flow: (flow.arriveT**2 * flow.bandwidth/(flow.sendT**5))),reverse=False) 

    threshold = len(ports) * 10#20的时候为30.1
    records = []
    while len(flows) > 0:            
        # 每装进去一个就更新时间
        waitList = flows.copy()
        waitListNum = len(waitList)
        count = 0
        ports.sort(key=(lambda port:(port.clock,1/(1+port.leftBandwidth))),reverse=False)
        for flow in waitList:
            flag = False
            for port in ports:
                if flow.bandwidth <= port.leftBandwidth:
                    flow.inputPort(port)
                    records.append(flow.getinfo())
                    flows.remove(flow)
                    flag = True
                    waitListNum -= 1
                    break
        ...
        if waitListNum > 0:
            ...
            ports.clear()
            flag = False
            for _,tmpPorts in portsClockDict.items():
                if not flag:
